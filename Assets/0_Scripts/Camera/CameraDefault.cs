...
public class CameraDefault : MonoBehaviour {
	...
	void LateUpdate () {
		...
		if (TargetLookAt == null)
			return;
		if (PlayerMgr.Instance.player.playerFSM.GetCurState () == E_FSM_State_Type.PlayerDead)
			return;
		if (PlayerMgr.Instance.player.transform.position.y >= 19) // 到达最高处
			return;
	...
	public Vector3 CalculatePosition (float rotationX, float rotationY, float distance) {
		...
		Quaternion rotation = Quaternion.Euler (rotationX + 20, rotationY + 45, 0);
